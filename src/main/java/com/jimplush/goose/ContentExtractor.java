 ...
 * and it held up well. Our current port is slightly different than this one but I'm working to align them so the com.jimplush.goose
...
public class ContentExtractor {
  ...
  public String extractContent(String rawHtml) {
      ParseWrapper parseWrapper = new ParseWrapper();
      return extractContent( new ParseWrapper().parse(rawHtml));
  }
  ...
  public String extractContent(Document doc) {
    ...
    try {
      ...
      topNode = calculateBestNodeBasedOnClustering(doc);

      if ((topNode = calculateBestNodeBasedOnClustering(doc)) != null) {
        ...
        text = outputFormatter.getFormattedText(topNode);
      }
    } catch (Exception e) {
