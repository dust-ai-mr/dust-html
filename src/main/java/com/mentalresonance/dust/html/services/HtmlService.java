/*
 *
 *  Copyright 2024-Now Alan Littleford
 *
...
public class HtmlService {
     ...
     */
    public static String wholeText(String html) {
    ...
    public static String extractStructuralText(String html) {
        Document doc = Jsoup.parse(html);

        // 1. Remove non-content tags completely
        doc.select("script, style, link, meta, noscript, template").remove();
        ...
        // 3. Remove all attributes (href, src, onclick, style, etc.)
        for (Element el : doc.getAllElements()) {
            el.clearAttributes();
        }

        // 4. Remove elements that contain *no visible text*
        removeNonTextElements(doc.body());

        // 5. Return cleaned HTML
        return doc.body().html().trim();
    }

    private static void removeNonTextElements(Element root) {
        for (Element el : root.select("*")) {

            // Get visible text (excluding whitespace)
